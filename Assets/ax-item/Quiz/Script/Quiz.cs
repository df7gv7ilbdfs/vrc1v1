...
public class Quiz : UdonSharpBehaviour
{
    ...
    public AudioSource question_audio;
    [Header("---------------")]
    public GameObject[] btn_group;
    public GameObject items_group, canvas, question_sound_btn;
    ...
    public AudioClip current_questionSound;
    public AudioClip[] current_answerSounds;
    ...
    public void pickUpNewQuestion()
    {
        ...
        current_answerSounds = (AudioClip[])item.GetProgramVariable("answer_sounds");
        current_questionSound = (AudioClip)item.GetProgramVariable("mainSound");
        question_sound_btn.SetActive(Utilities.IsValid(current_questionSound));
        bool isShuffle = (bool)item.GetProgramVariable("isShuffle");
        for (int i = 0; i < btn_group.Length; i++)
        {
            GameObject obj = btn_group[i];
            UdonBehaviour udon = (UdonBehaviour)obj.transform.Find("Answer").GetComponent(typeof(UdonBehaviour));
            TextMeshProUGUI tm = (TextMeshProUGUI)obj.transform.Find("Answer/Text").GetComponent(typeof(TextMeshProUGUI));
            GameObject soundBtnObj = obj.transform.Find("Sound").gameObject;
            if (i < ans_l.Length)
            {
                string temp = ans_l[i];
                ...
                tm.text = temp;
                obj.SetActive(true);
                if (current_answerSounds.Length > i)
                {
                    soundBtnObj.SetActive(Utilities.IsValid(current_answerSounds[i]));
                }
                else
                {
    ...
    {
        question_audio.Stop();
        question_audio.PlayOneShot(current_questionSound);
    }
