...
public class ObjectSerializer
{
    ...
    private static string Serialize_Internal(object obj)
    {
        ...
        {
            return TokenType_String + ":" + Pack((string)obj);
        }
        ...
        else if(type == typeof(DataDictionary[]))
        {
            ...
            for(var i = 0;i < arr.Length;i++)
            {
                ...
                {
                    _arg_str += SerializeDataToken(keys[j]) + "?" + SerializeDataToken(arr[i][keys[j]]);
    ...
    private static object Deserialize_Type(string context, int tokenType, int subTokenType, string arg_str)
    {
        ...
        switch(tokenType)
        {
            ...
            case TokenType_Array:
            {
                ...
                if(array_items[1].Length == 0)
                {
                    switch(_tokenType)
                    {
                        case TokenType_Null:
                            ThrowExceptionWithContext(context, "TokenType can't be Null: " + _tokenType + ":" + arg_str);
                            return null;
                        ...
                        default:
                            ThrowExceptionWithContext(context, "Unknown TokenType " + _tokenType + ": " + arg_str);
                            return null;
                    }
                }
                ...
                switch(_tokenType)
                {
                    ...
                    case TokenType_DataList:
                    {
                        ...
                        for(var i = 0;i < elements.Length;i++)
                        {
                            arr[i] = (DataList)Deserialize_Type(context, _tokenType, TokenType_None, elements[i]);
                        }
                    ...
                    case TokenType_DataDictionary:
                    {
                        ...
                        for(var i = 0;i < elements.Length;i++)
                        {
                            arr[i] = (DataDictionary)Deserialize_Type(context, _tokenType, TokenType_None, elements[i]);
                        }
                    ...
                    case TokenType_DataToken:
                    {
                        ...
                        for(var i = 0;i < elements.Length;i++)
                        {
                            var arguments = ParseArguments(context, _tokenType + ":" + elements[i]);
